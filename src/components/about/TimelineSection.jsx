...
const DesktopTimelineItem = ({ year, title, description, isLeft }) => {
  return (
    ...
    >
      ...
      {/* Content */}
      <div
        className={`w-5/12 ${isLeft ? "text-right pr-12" : "text-left pl-12"}`}
      >
        <span className="text-indigo-400 font-bold block mb-1">{year}</span>
        <h3 className="text-lg font-bold mb-2">{title}</h3>
        <p className="text-slate-300 text-sm">{description}</p>
  ...
  const timeline = [
    {
      ...
      description:
        "Connecting Dots Innovations was established with a mission to bridge the gap between advanced software and hardware solutions for emerging tech challenges.",
    },
    {
      year: "2023",
      title: "First Major Client",
      description:
        "Secured partnership with a Fortune 500 company to develop custom IoT solutions, marking our breakthrough into enterprise-level projects.",
    },
    {
      year: "2024",
      title: "Expansion to AI Solutions",
      description:
        "Launched our specialized AI division, focusing on machine learning integration and developing cutting-edge predictive analytics platforms.",
    },
    {
      year: "2024",
      title: "New Office Opening",
      description:
        "Expanded operations with a new headquarters in Indore's tech corridor, providing space for our growing team and innovation lab.",
    },
    {
      year: "2025",
      title: "International Partnerships",
      description:
        "Formed strategic partnerships with international tech firms to collaborate on cross-border technology initiatives and expand our global presence.",
    },
  ];
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-12">
          ...
          <p className="max-w-2xl mx-auto text-slate-300">
            The key milestones that have defined our growth and success.
          </p>
        </div>
        ...
        <div className="md:hidden relative mx-auto max-w-md">
          {timeline.map((item, index) => (
            <MobileTimelineItem key={index} {...item} />
          ))}
        </div>
